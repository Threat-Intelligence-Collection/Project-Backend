...
): Promise<Either<ApiResponse, GoogleUser>> {
...
async function GetgoogleOAuthCode(
  req: GoogleOAuthRequest
...
async function GetgoogleUser(
  access_token: string
): Promise<Either<string, GoogleUser>> {
  const response = (await axios.get(`${process.env.GOOGLE_USER_INFO_URL}`, {
    headers: {
